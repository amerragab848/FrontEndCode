...
const validationSchema = Yup.object().shape({
  companyId: Yup.string().required(language["fromCompanyRequired"][currentLanguage]),
  contactId: Yup.string().required(language["fromContactRequired"][currentLanguage])
});
...
class DashboardProject extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      projectId: this.props.projectId == 0 ? localStorage.getItem('lastSelectedProject') : this.props.projectId,
      selectedFromCompany: { label: language.fromCompanyRequired[currentLanguage], value: "0" },
      selectedFromContact: { label: language.fromCompanyRequired[currentLanguage], value: "0" },
      Discussions: [],
      viewComment: false,
  ...
  componentDidMount() {
    ...
    dataService.GetDataGrid("GetDiscussions?projectId=" + this.state.projectId).then(result => {
  ...
  renderWidget(widget, index) {
    ...
    Details.widgets[widget.title].props.api = Details.widgets[widget.title].props.api + this.state.projectId;
      ...
      return (
        <Fragment>
          <div className="tree__header">
            <h2 className="zero">{language.organizationChart[currentLanguage]}</h2>
                    ...
                    <Fragment key={i}>
                      <div className={this.state[item.id] == -1 ? ' epsTitle' : this.state['_' + item.id] === true ? 'epsTitle active' : 'epsTitle'} key={item.id}
                        ...
                        <div className="listTitle">
                          <span className="dropArrow" style={{ visibility: (item.charts.length > 0 ? '' : 'hidden') }}>
                            <i className="dropdown icon" />
                          </span>
                          <span className="accordionTitle" onClick={this.props.GetNodeData ? () => this.GetNodeData(item) : null}>{this.state[item.id] ? `${this.state[item.id].contactName} - ${this.state[item.id].companyName}` : `${item.contactName} - ${item.companyName}`}
                          </span>
                        </div>
                      ...
                      <div className="epsContent" id={item.id}>
                        {item.charts.length > 0 ? this.printChild(item.charts) : null}
                      </div>
                    </Fragment>
                  )
                })
          ...
          <Fragment key={i}>
            <div className={this.state[item.id] == -1 ? ' epsTitle' : this.state['_' + item.id] === true ? 'epsTitle active' : 'epsTitle'} key={item.id} onClick={() => this.viewChild(item)}
              ...
              <div className="listTitle">
                ...
                <span className="accordionTitle" onClick={this.props.GetNodeData ? () => this.GetNodeData(item) : null}>{this.state[item.id] ? this.state[item.id].companyName : item.companyName}
                </span>
              </div>
              {this.props.showActions == false ? null :
                <div className="Project__num">
  ...
  saveTree() {
    ...
    dataService.addObject("AddNewProjectOrganizationChart", saveDocument).then(result => {
  ...
  editTree = () => {
    ...
    dataService.addObject("EditProjectOrganizationChart", saveDocument).then(result => {
      toast.success(language["operationSuccess"][currentLanguage]);
  ...
  clickHandlerContinueMain() {
    ...
    dataService.addObject("ProjectOrganizationChartDelete?id=" + this.state.docId).then(result => {
      let state = this.state
  ...
  EditDocument(item) {
    this.setState({
      ...
      document: item,
      mode: 'goEdit',
      selectedFromCompany: { label: item.companyName, value: item.companyId },
      selectedFromContact: { label: item.contactName, value: item.contactId }
    });
    this.simpleDialog.show();
  }
  ...
  addComments = () => {
    ...
    if (this.state.documentCommentDiscussion.message != "") {
      ...
      dataService.addObject("AddDiscussionComment", this.state.documentCommentDiscussion).then(result => {
        ...
        let indexDiscussionId = originalData.findIndex(x => x.id === DiscussionId);
  ...
  renderDiscussions() {
    return (
      <div className="discussion__panel">
        <div className="post__panel" onBlur={() => this.focusBtn("Blur")}>
          <div className="post__panel--box">
            <div className="post__input">
              <figure className="zero avatarProfile ">
                <img alt="" title="" src={this.state.profilePath} />
              </figure>
              <input id="discussion__input" type="text" className="form-control " placeholder="What’s happening?" onChange={() => this.focusBtn("change")} />
            </div>
            {this.state.preview ?
              <div id="preview" className={this.state.filesupload ? 'file_upload' : ''}>
                {this.state.filesupload ?
                  <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="60" height="66" viewBox="0 0 60 66">
                    <g fill="#CCD2DB" fill-rule="evenodd" transform="translate(-246 -72)">
                      <path id="a" d="M305.045 90.939c.728.571 1.04 1.333.936 2.19l.019 37.538c0 3.43-2.743 7.333-7.5 7.333h-45c-4.565 0-7.5-3.6-7.5-7.333V79.333c0-4.77 4.267-7.333 7.5-7.333h30.665c.935 0 1.767.286 2.39.857l18.49 18.082zm-28.917-5.59c0-3.29-2.826-6.016-6.237-6.016H253.5v51.334h45v-23.152c0-2.588-2.033-6.182-6.27-6.182h-12.984c-1.754 0-3.118-1.316-3.118-3.008V85.35zM261 116h22.5v7.333H261V116z"></path>
                    </g>
                  </svg> : null}
              </div> : null}
            <div className="post__attachment--container">
              <div className="post__attachment">
                ...
                <form className="file__upload" method="post" encType="multipart/form-data">
                  ...
                  <input className="input__dissUpload" type="file" name="file" id="videoInput" onChange={this.updateVideoDisplay} onClick={() => this.setState({ preview: true, filesupload: false })} accept="video/*" className="inputfile" />
                  <label htmlFor="file">{language.video[currentLanguage]}</label>
                </form>
                <form className="file__upload" method="post" encType="multipart/form-data">
                  <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="24" height="24" viewBox="0 0 24 24">
                    <g fill="none" fillRule="evenodd" transform="translate(5 5)">
                      <g fill="#4382F9" mask="url(#b)">
                        <path id="a" d="M13.42 4.304c.165.13.236.303.212.498l.004 8.531c0 .78-.623 1.667-1.704 1.667H1.705A1.67 1.67 0 0 1 0 13.333V1.667C0 .583.97 0 1.705 0h6.969c.212 0 .401.065.543.195l4.202 4.11zm-6.573-1.27c0-.748-.642-1.367-1.417-1.367H1.705v11.666h10.227V8.072c0-.589-.462-1.405-1.425-1.405H7.556c-.399 0-.709-.3-.709-.684V3.034zM3.41 9.6h5.114v1.87H3.409V9.6z" />
                      </g>
                    </g>
                  </svg>
                  <input className="input__dissUpload" type="file" name="file" id="filesInput" onChange={this.updateFileDisplay} onClick={() => this.setState({ preview: true })}
                    accept=".doc,.docx,.pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    className="inputfile" />
                  <label htmlFor="file">{language.document[currentLanguage]}</label>
                </form>
              </div>
              <button className={"primaryBtn-1 btn smallBtn " + (this.state.viewBtnDiscussion === true ? " disabled " : "")}>{language.adddiscussion[currentLanguage]}</button>
            </div>
          </div>
        </div>
        <div className="timeline">
          {/* overlay */}
          <div className={"timeline__month--container " + (this.state.viewComment === true ? "overlay " : "")} id="august">
            ...
            {this.state.Discussions.map((item, index) => {
              return (
                //  overlay 
                <div className={"timeline__posts " + (this.state.rowPosts === index ? " overlay " : "")} key={index} tabIndex="1" onBlur={() => this.viewComments(item.comments.length, index)}>
                  <div className="timeline__posts--header">
                    <figure className="zero avatarProfile ">
                      <img alt="" title="" src={item.image} />
                    </figure>
                    <div className="header__info">
                      <h2 className="zero">{item.name} <span>@Procoor</span></h2>
                      <p className="zero">3 minutes ago</p>
                    </div>
                  </div>
                  <div className="timeline__posts--body">
                    <p className="zero">{item.message}</p>
                    <div className="items__uploades">
                      <img src={item.attachment} alt="Discussion" />
                    </div>
                  </div>
                  <div className="timeline__posts--comments">
                    <div className="comment_number">
                      ...
                      <span onClick={() => this.viewComments(item.comments.length, index)}>{item.comments.length} Comments</span>
                      {/* active */}
                      <div className={"comments__number--list " + (this.state.rowPosts === index ? " active " : "")}>
                        {
                          item.comments.length > 0 ?
                            item.comments.map((comments, index) => {
                              return (
                                <div className="timeline__posts" key={index}>
                                  <div className="timeline__posts--header">
                                    <figure className="zero avatarProfile ">
                                      <img alt="" title="" src={comments.image} />
                                    </figure>
                                    <div className="header__info">
                                      <h2 className="zero">{comments.name} <span>@Procoor</span></h2>
                                      <p className="zero">3 minutes ago</p>
                                    </div>
                                  </div>
                                  <div className="timeline__posts--body">
                                    <p className="zero">{comments.message}</p>
                                  </div>
                                </div>
                              )
                            })
    ...
    return (
      <div className="customeTabs">
        <Tabs selectedIndex={this.state.tabIndex} onSelect={tabIndex => this.onClickTabItem(tabIndex)}>
          ...
          <TabPanel><div className="mainContainer">{this.state.viewWidget ? this.renderCategoryWidget() : null}</div></TabPanel>
          <TabPanel><div className="mainContainer">{this.renderDiscussions()} </div></TabPanel>
          <TabPanel><div className="mainContainer">{this.renderOrganizationChart()}</div></TabPanel>
        </Tabs>
            ...
            <Formik initialValues={{ ...this.state.document }} validationSchema={validationSchema}
              ...
              {({ errors, touched, handleBlur, handleChange, values, handleSubmit, setFieldTouched, setFieldValue }) => (
                <Form className="dropWrapper proForm" onSubmit={handleSubmit}>
                  <Dropdown title="company" data={this.state.companies} selectedValue={this.state.selectedFromCompany}
                    handleChange={event => {
                  ...
                  <Dropdown title="fromContact" data={this.state.contacts} selectedValue={this.state.selectedFromContact}
                    handleChange={event => {
                      this.handleChangeDropDown(event, "contactId", true, "contacts", "GetContactsByCompanyId", "contactId", "selectedFromContact", "");
                    }}
                    onChange={setFieldValue}
                    onBlur={setFieldTouched}
        ...
        {this.state.showDeleteModal == true ? (
          <ConfirmationModal
            title={language["smartDeleteMessage"][currentLanguage].content}
            buttonName="delete"

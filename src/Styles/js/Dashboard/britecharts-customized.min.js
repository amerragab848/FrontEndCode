...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
    ...
    var wrapText = function wrapText(xOffset, fontSize, availableWidth, node) {
        var text = d3Selection.select(node),
            ...
            tspan = text.text(null).append('tspan').attr('x', -10).attr('y', y - 30).attr('dy', dy + 'em').classed(wrapConfig.valueClassName, true).style('font-size', fontSize + 'px');

        tspan.text(words.pop());
        tspan = text.append('tspan').classed(wrapConfig.labelClassName, true).attr('x', '250').attr('y', y + wrapConfig.smallTextOffset).attr('dy', ++lineNumber * smallLineHeight + dy + 'em').style('font-size', smallFontSize + 'px');
                tspan.text(words.pop());
        tspan = text.append('tspan').classed(wrapConfig.labelClassName, true).attr('x', '250').attr('y', '20').attr('dy', ++lineNumber * smallLineHeight + dy + 'em').style('font-size', smallFontSize + 'px');
// hossam tspan
...
//hossam
                ...
                }).attr('dy', '-1em').attr('text-anchor', 'middle').attr('dx', '15em');
...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
    ...
    return function module() {
        ...
        colorScale = void 0,
            ...
            return d3Format.format(_ref3.quantity)(quantity) + unit + "LE";
        },
            //point
         ...
         */
        function buildContainerGroups() {
            var container = svg.append('g').classed('legend-container-group', true).attr('transform', 'translate(20, 20)');
        ...
        function cleanFadedLines() {
            svg.select('.legend-group').selectAll('g.legend-entry').classed(isFadedClassName, false).classed(isNoFadedClass, false);
        }
        ...
        function drawHorizontalLegend() {
            ...
            // Enter
            entries.enter().append('g').classed('legend-entry', false).attr('data-item', getId).merge(entries).append('circle').classed('legend-circle', true).attr('cx', markerSize / 2).attr('cy', markerYOffset).attr('r', markerSize / 2).style('fill', getCircleFill).style('stroke-width', 1);

            svg.select('.legend-group').selectAll('g.legend-entry').append('text').classed('legend-entry-name', true).text(getName).style('font-size', textSize + 'px').style('letter-spacing', textLetterSpacing + 'px');
        ...
        function drawVerticalLegend() {
            ...
            // Enter
            entries.enter().append('g').classed('legend-line', true).append('g').classed('legend-entry', true).attr('data-item', getId).merge(entries).append('circle').classed('legend-circle', true).attr('cx', markerSize / 2).attr('cy', markerYOffset).attr('r', markerSize / 2).style('fill', getCircleFill).style('stroke-width', 1);
        ...
        function fadeLinesBut(exceptionItemId) {
            ...
            if (entryLine.nodes().length) {
                svg.select('.legend-group').selectAll(classToFade).classed(isFadedClassName, true).classed(isNoFadedClass, false);

                entryLine.classed(isFadedClassName, false).classed(isNoFadedClass, true);
            }

...
class ViewAttachmments extends Component {
    ...
    async createPdfVersion(obj) {
        ...
        var prevVersions = lodash.filter(documentAttachmentsChilds, function (o) { return lodash.lt(o.createdDate, obj.createdDate) && !o.isMain && o.accountId !== obj.accountId });
        ...
        let res = await fetch("https://cjpgr4qmpe.execute-api.eu-west-1.amazonaws.com/v1/get-merges",
            {
                method: "POST",
                body: JSON.stringify({ fileId: documentAttachmentsChilds[0].id }),
                headers: {
        ...
        if (responseJson && responseJson.length && responseJson[0].merges) {
            ...
            for (var i = 0; i < docs.length; i++) {
                pdfDocs.push(
                    await PDFLib.PDFDocument.load(docs[i].byteArray),
                );
        ...
        for (var avCount = 0; avCount < appliedVersions.length; avCount++) {
            ...
            if (version.data.length && !version.data[0].mods) {
                version.data.forEach(obj => {
                    obj.json.objects = obj.json.objects.map(
                        cvsObj => {
                            var uuid = uuidv1();
                            ...
                            if ((['text', 'i-text'].indexOf(cvsObj.type) !== -1)) {
                                customizedCvsObj = Object.assign(customizedCvsObj, { scaleX: 1, scaleY: 1 });
                            }

                            if (['text', 'i-text'].indexOf(cvsObj.type) !== -1 && ['✓', '✗'].indexOf(cvsObj.text) !== -1) {
                    ...
                    mergedVersions[obj.page.toString()] = mergedVersions[obj.page.toString()] ? [...mergedVersions[obj.page.toString()], ...obj.json.objects,] : [...obj.json.objects];
                });
            }
        }
        ...
        Promise.all(promises).then(async (values) => {
                ...
                if (cvs._objects && cvs._objects.length) {
                    changes.push({
                        svgs: cvs._objects.map((cvs_obj, i) => {
                            ...
                            if (
                                ['text', 'i-text'].indexOf(cvs_obj.type) !== -1 &&
                                cvs_obj.angle
                            ) {
                            ...
                            return {
                                ...
                                imgType:
                                    cvs_obj.type === 'image'
                                        ? cvs_obj.getSrc().indexOf('.jpg') !== -1 ||
                                            cvs_obj.getSrc().indexOf('.jpeg') !==
                                            -1 ||
                                            cvs_obj.getSrc().indexOf('image/jpeg') !==
                                            -1
                                            ? '.jpg'
                                            : '.png'
                                        : null,
            ...
            var notoEmojiBuffer = await fetch('/assets/fonts/NotoEmoji-661A.ttf').then(
                res => res.arrayBuffer(),
            );
                ...
                try {
                    ...
                    switch (rotation) {
                        case 0:
                            pdfpage.pushOperators(
                                PDFLib.concatTransformationMatrix(1, 0, 0, 1, 0, 0),
                            );
                            break;
                        case 90:
                            pdfpage.pushOperators(
                    ...
                    for (let i2 = 0; i2 < cvs.svgs.length; i2++) {
                        try {
                            ...
                            if (['text', 'i-text', 'line'].indexOf(cvs_obj.type) === -1) {
                                if (cvs_obj.type === 'image') {
                                    ...
                                    if (
                                        ...
                                        -1 ||
                                        cvs_obj.imageSrc.indexOf('http://') !== -1
                                    ) {
                                        buffer = await fetch(
                                    ...
                                    if (embedFunc) {
                                        let image = await currentPDFDoc[embedFunc](
                                            buffer,
                                        );
                                    ...
                                    let svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">${cvs_obj.svg
                                        }</svg>`;
                                ...
                                pdfpage.drawLine({
                                    ...
                                    start: {
                                        ...
                                        y: Math.abs(height) - cvs_obj.lineCoords.y1,
                                    },
                                    end: {
                                        x: cvs_obj.lineCoords.x2,
                                        y: Math.abs(height) - cvs_obj.lineCoords.y2,
                                    },
                                ...
                                let textHeight =
                                    cvs_obj.fontFamily === 'Noto Emoji'
                                        ? cvs_obj.height - 4
                                        : fontFamily.heightAtSize(cvs_obj.fontSize);
    ...
    getChildVersions(parentId) {
        ...
        return fetch('https://cjpgr4qmpe.execute-api.eu-west-1.amazonaws.com/v1/get-versions',
            {
                method: 'POST',
    ...
    render() {
                    ...
                    <td className="tdHover">
                        <div className="attachmentAction">
                            <a
                                ...
                                className="pdfPopup various zero attachPdf" data-toggle="tooltip" title={Resources["download"][currentLanguage]}>
        ...
        let tabel = this.props.isLoadingFiles == true
            ? this.props.files.map((item, Index) => {
                ...
                let attachFile = item["attachFile"].replace('?dl=0', '?dl=1');
                if (item.fileName) {
                    ...
                    <tr key={Index}>
                        ...
                        <td className="tdHover">
                            <div className="attachmentAction">
                                ...
                                <a {...(ext === 'pdf' ? { onClick: e => this.downloadLatestPdfVersion(item) } : { href: attachFile, download: item.fileName })} className="pdfPopup various zero attachPdf" data-toggle="tooltip" title={Resources["download"][currentLanguage]}>
                                ...
                                {Config.IsAllow(4501) ? (
                                    <a className="attachPend"
                                        onClick={() => this.goToVersions(item)}
                                        data-toggle="tooltip" title={Resources["versions"][currentLanguage]}>
        ...
        return (
            ...
            <React.Fragment>
                ...
                <div className="largePopup largeModal "
                    style={{ display: this.state.viewVersion ? "block" : "none" }}>

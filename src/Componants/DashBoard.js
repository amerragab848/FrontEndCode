...
class DashBoard extends Component {
  ...
  async componentDidMount() {
    ...
    types[1].categories.forEach(category => {
      ...
      type[category.id] = {
        order: types[1].categories.find(cat => cat.id === category.id).widgets.map(widget => widget.order.toString())
      };
    ...
    types[2].categories.forEach(category => {
      ...
      type[category.id] = {
        order: types[2].categories.find(cat => cat.id === category.id).widgets.map(widget => widget.order.toString())
      };

      widgetOrders[3] = type;
    });

    this.setState({ types, widgets, categoryOrder, widgetOrders, selected });
  }
  ...
  async toggleCheckAll(typeId, categoryId) {
    ...
    let widgets = this.state.types[typeId - 1].categories.find(category => category.id === categoryId).widgets;

    let selectedCategory = selected[categoryId];

    for (let index = 0; index < widgets.length; index++) {
      if (selectedCategory.length === widgets.length) {
        await IndexedDb.update('widget', widgets[index].id, { checked: false });
        ...
        if (selectedCategory.indexOf(widgets[index].id) === -1) {
          ...
          await IndexedDb.update('widget', widgets[index].id, { checked: true });
        }
      }
    }
  ...
  changeWidgetCategoryOrder(order) {
    ...
    new_state.widgetOrders[this.state.type][this.state.category].order = order;
  ...
  async widgetOrderChanged(order) {
    ...
    if (this.state.types && this.state.types[this.state.type]) {
      ...
      let widgets = types[this.state.type - 1].categories.find(category => category.id === this.state.category).widgets
      ...
      for (let index = 0; index < widgets.length; index++) {
        ...
        new_order = +`${this.state.type}${widget.categoryId}${new_order + 1}`;
  ...
  async updateOrder(category) {
    return await IndexedDb.update('widgetCategory', category.id, { order: category.order });
  }

  async updateWidgetOrder(widget) {
    return await IndexedDb.update('widget', widget.id, { order: widget.order });
  }
          ...
          <Pane key={category.order} defaultSize={{ width: "50%" }} resizable={{ x: false, y: false, xy: false }}>
            <div className="secondTabs project__select ui-state-default">
              ...
              <div className={checked ? "ui checkbox checkBoxGray300 count checked" : "ui checkbox checkBoxGray300 count"}
                onClick={event => { this.openCategory(category.id); this.toggleCheckAll(category.typeId, category.id); }}>
  ...
  render() {
    ...
    let currentCategoryChecked =
      this.state.showWidgets ? this.state.selected[this.state.category].length === this.state.types[this.state.type - 1].categories
        .find(category => category.id === this.state.category).widgets.length : false;

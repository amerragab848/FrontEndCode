...
class uploadSignture extends  Component {
  constructor(props) {
    super(props);
    this.state = {
      sign: {},
      signPreview: {},
      signShowRemoveBtn: false,
      signName: "",
      signIamge: config.getPublicConfiguartion().downloads + signiturePath,
      profile: {},
      profilePreview: {},
      profileShowRemoveBtn: false,
      profileName: "",
      ...
      <div className="mainContainer">
        <div>
          <section className="singleUploadForm">
                ...
                {this.state.signName ? (
                  <div className="thumbStyle" key={this.state.signName}>
                    <div className="thumbInnerStyle">
                      <img src={this.state.signPreview} className="imgStyle" />
                    </div>
                  </div>
                ) : null}
              </aside>
            ) : null}
            <Dropzone accept="image/*" onDrop={this.onDropSign.bind(this)}>
              {({ getRootProps, getInputProps }) => (
                <div className="singleDragText" {...getRootProps()}>
                  <input {...getInputProps()} />

                  {this.state.signName ? null : (
                    <p>{resources["dragFileHere"][currentLanguage]}</p>
                  )}
                  <button className="primaryBtn-1 btn smallBtn">
                    {resources["chooseFile"][currentLanguage]}
                  </button>
                </div>
              )}
            </Dropzone>
            {this.state.signName ? (
              <div className="removeBtn">
                <button className="primaryBtn-2 btn smallBtn" onClick={this.RemoveHandlerSign}>
                  {resources["clear"][currentLanguage]}
                </button>
              </div>
            ) : null}
          </section>
          ...
          <div className="fullWidthWrapper">
            <button className="primaryBtn-1 btn smallBtn" onClick={this.uploadSign}>
              {resources["uploadSignature"][currentLanguage]}
            </button>
          </div>
          <div className="a7medImg">
            {this.state.signName ? <img src={this.state.signIamge} /> : null}
          </div>
        </div>
        <hr />
        <div>
          <section className="singleUploadForm">
              ...
              <aside className="thumbsContainer">
                ...
                {this.state.profileName ? (
                  <div className="thumbStyle" key={this.state.profileName}>
                    <div className="thumbInnerStyle">
                      <img src={this.state.profilePreview} className="imgStyle"/>
                    </div>
                  </div>
                ) : null}
              </aside>
            ) : null}
            <Dropzone accept="image/*" onDrop={this.onDropPP.bind(this)}>
              {({ getRootProps, getInputProps }) => (
                <div className="singleDragText" {...getRootProps()}>
                  <input {...getInputProps()} />
            ...
            {this.state.profileName ? (
              <div className="removeBtn">
                <button className="primaryBtn-2 btn smallBtn" onClick={this.RemoveHandlerPP}>
                  {this.RemoveHandlerSign}
                  {resources["clear"][currentLanguage]}
                </button>
              </div>
            ) : null}
          ...
          <div className="fullWidthWrapper">
            <button className="primaryBtn-1 btn smallBtn" onClick={this.uploadPP}>
              {resources["uploadPhoto"][currentLanguage]}
            </button>
          </div>

          <div className="a7medImg">
            {this.state.profileName ? (
              <img src={this.state.profileIamge} />
            ) : null}
          </div>
        </div>
      </div>

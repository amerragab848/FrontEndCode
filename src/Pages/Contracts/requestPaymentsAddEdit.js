...
class requestPaymentsAddEdit extends Component {
    ...
    buildColumns(changeStatus) {
        ...
        var currentGP = [];
        if (selectedCols != null && selectedCols != "undefined") {
            if (selectedCols.length === 0) {

                var gridLocalStor = { columnsList: [], groups: [] };

                gridLocalStor.columnsList = JSON.stringify(itemsColumns);
                gridLocalStor.groups = this.props.groups;
                currentGP = this.props.groups;
                localStorage.setItem("ReqPaymentsItems", gridLocalStor);
            }
            else {

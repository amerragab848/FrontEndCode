...
class projectBackLog extends Component {
    ...
    getData = () => {
        ...
        Api.post('GetTranfarePerProject', reportobj).then(
            res => {
                this.setState({
                    rows: res,
                    isLoading: false,
    ...
    render() {
        ...
        const btnExport = this.state.isLoading === false ?
            <ExportDetails
                ...
                rows={this.state.rows} fields={this.fields} fileName={Resources.transfarePerProject[currentLanguage]} /> : null

        return (
            <div className="reports__content">
                <header>
                    <h2 className="zero">{Resources.transfarePerProject[currentLanguage]}</h2>
                    {btnExport}
                </header>

...
const documentItemValidationSchema = Yup.object().shape({
    ...
    arrange: Yup.number().typeError(Resources['onlyNumbers'][currentLanguage])
        .required(Resources['selectNumber'][currentLanguage]),

    quantity: Yup.number().typeError(Resources['onlyNumbers'][currentLanguage])
        .required(Resources['quantityRequired'][currentLanguage]),

    revQuantity: Yup.number().typeError(Resources['onlyNumbers'][currentLanguage])
...
class procurementAddEdit extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            IsAddMood: false,
            SupplierData: [],
            selectedDisciplineId: { label: Resources.disciplineRequired[currentLanguage], value: "0" },
            selectedSupplier: { label: Resources.selectSupplier[currentLanguage], value: "0" },
            itemTypes: [],
    ...
    componentWillReceiveProps(nextProps, prevProps) {
        if (nextProps.document.id) {
            ...
            dataservice.GetDataGrid('GetContractsProcurementContractors?procurmentId=' + doc.id).then(
                res => {
                    this.setState({ SupplierData: res })
                }
            )

            dataservice.GetDataGrid('GetProcurmentItemCustomizeTable?procurmentId=' + doc.id).then(
                res => {
    ...
    componentWillMount() {
        if (this.state.docId > 0) {
            ...
            const Document = {
                projectId: projectId, status: "true", companyId: "", subject: "",
                docDate: moment(), disciplineId: ''
            }
        ...
        if (this.props.changeStatus === true) {
            ...
            if (this.state.isApproveMode != true && Config.IsAllow(167)) {
                if (this.props.hasWorkflow == false && Config.IsAllow(167)) {
                    if (this.props.document.status !== false && Config.IsAllow(167)) {
    ...
    saveAndExit(event) {
        if (this.state.CurrStep === 1) { this.setState({ CurrStep: this.state.CurrStep + 1 }) }
        else { this.props.history.push("/procurement/" + this.state.projectId) }
    }
    ...
    AddingSuppliersAddition = (values) => {
        ...
        let SaveObj = {
            ...
            notes: values.notes, procurementId: this.state.docId
        }
        dataservice.addObject('AddContractsProcurementContractors', SaveObj).then(
            result => {
                values.notes = ''
                values.contractorId = ''
                this.setState({
                    selectedSupplier: { label: Resources.selectSupplier[currentLanguage], value: "0" },
                    SupplierData: result, isLoading: false
                })
                toast.success(Resources["operationSuccess"][currentLanguage])
    ...
    SaveItem = (values) => {
        ...
        let SaveObj = {
            ...
            equipmenttypeId: this.state.selectedEquipmentType.value === '0' ? undefined : this.state.selectedEquipmentType.value,
            resourceCode: values.resourceCode,
            arrange: values.arrange,
            action: this.state.action
        }
        dataservice.addObject('AddContractsProcurementItems', SaveObj).then(
            res => {
                values.description = ''
                values.quantity = ''
                values.revQuantity = ''
                ...
                values.days = 1
                this.setState({
                    isLoading: false, procurementItems: res,
                    selectedItemType: { label: Resources.itemTypeSelection[currentLanguage], value: "0" },
                    selectedEquipmentType: { label: Resources.equipmentTypeSelection[currentLanguage], value: "0" },
                    selectedSpecsSection: { label: Resources.specsSectionSelection[currentLanguage], value: "0" },
                })
    ...
    fillTable = () => {
        this.setState({ isLoading: true })
        dataservice.GetDataGrid('GetContractsProcurementItems?procurmentId=' + this.state.docId).then(
            res => {
                        ...
                        {({ errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue, setFieldTouched }) => (
                            <Form id="QsForm" className="customProform" noValidate="novalidate" onSubmit={handleSubmit}>
                                ...
                                <div className="proForm datepickerContainer">
                                    ...
                                    <div className="linebylineInput valid-input">
                                        <Dropdown title="descipline" data={this.state.disciplineData} selectedValue={this.state.selectedDisciplineId}
                                            handleChange={event => this.handleChangeDropDown(event, "disciplineId", false, "selectedDisciplineId")}
                                ...
                                <div className="doc-pre-cycle letterFullWidth">
                                    <div>
                                        {this.state.docId > 0 ?
                                            <UploadAttachment changeStatus={this.props.changeStatus} AddAttachments={942} EditAttachments={3272} ShowDropBox={3547}
                                                ShowGoogleDrive={35428} docTypeId={this.state.docTypeId} docId={this.state.docId} projectId={this.state.projectId} />
                                            : null}
            ...
            return (
                <div className="step-content">
                    ...
                    <div className={"subiTabsContent feilds__top " + (this.props.isViewMode ? "readOnly_inputs" : " ")}>
                                        ...
                                        <div className="proForm first-proform">
                                            <div className="linebylineInput valid-input letterFullWidth ">
                                                ...
                                                <div className={"inputDev ui input" + (errors.description && touched.description ? (" has-error") : !errors.description && touched.description ? (" has-success") : " ")} >
                                                    <input name='description' className="form-control fsadfsadsa" id="description" autoComplete='off'
                                        ...
                                        <div className="proForm datepickerContainer">
                                            ...
                                            <div className="linebylineInput valid-input">
                                                <Dropdown title="specsSection" data={this.state.SpecsSectionData} selectedValue={this.state.selectedSpecsSection}
                                                    handleChange={event => this.setState({ selectedSpecsSection: event })}
                                            ...
                                            <div className="linebylineInput valid-input">
                                                ...
                                                <div className={"inputDev ui input " + (errors.quantity ? 'has-error' : !errors.quantity && touched.quantity ? (" has-success") : " ")}>
                                                    <input className="form-control" name='quantity' value={values.quantity}
                                                        placeholder={Resources['quantity'][currentLanguage]} onChange={handleChange}
                                                        onBlur={(e) => {
                                                            handleBlur(e)
                                                            handleChange(e)
                                                        }} />
                                            ...
                                            <div className="linebylineInput valid-input">
                                                ...
                                                <div className={"inputDev ui input " + (errors.arrange ? 'has-error' : !errors.arrange && touched.arrange ? (" has-success") : " ")}>
                                                    <input className="form-control" name='arrange' value={values.arrange}
                                                        placeholder={Resources['no'][currentLanguage]} onChange={handleChange}
                                                        onBlur={(e) => {
                                                            handleBlur(e)
                                                            handleChange(e)
                                                        }} />
                                            ...
                                            <div className="linebylineInput valid-input">
                                                <label className="control-label">{Resources['revQuantity'][currentLanguage]} </label>
                                                <div className={"inputDev ui input " + (errors.revQuantity ? 'has-error' : !errors.revQuantity && touched.revQuantity ? (" has-success") : " ")}>
                                                    <input className="form-control" name='revQuantity'
                                                        placeholder={Resources['revQuantity'][currentLanguage]} value={values.revQuantity}
                                                        onChange={handleChange}
                                                        onBlur={(e) => {
                                                            handleBlur(e)
                                                            handleChange(e)
                                                        }} />
                                                    {errors.revQuantity ? (<em className="pError">{errors.revQuantity}</em>) : null}
                                                </div>
                                            </div>
                        ...
                        <XSLfile key='procurementitem' docId={this.state.docId} docType={this.state.docType} link={IPConfig.downloads + '/DownLoads/Excel/procurement.xlsx'} header='addManyItems'
                            disabled={this.props.changeStatus ? (this.props.docId === 0 ? true : false) : false} afterUpload={() => this.fillTable()} />
                        <div className="doc-pre-cycle">
                            <header>
                                <h2 className="zero">{Resources['procurementItems'][currentLanguage]}</h2>
                            </header>
                                ...
                                <Form onSubmit={handleSubmit}>
                                    ...
                                    <div className='document-fields'>
                                        ...
                                        <div className="proForm datepickerContainer">
                                            ...
                                            <div className="linebylineInput valid-input">
                                                <Dropdown title="suppliers" data={this.state.FromCompaniesData} selectedValue={this.state.selectedSupplier}
                                                    handleChange={e => this.setState({ selectedSupplier: e })}
                                                    onChange={setFieldValue} onBlur={setFieldTouched} error={errors.contractorId}
                                                    touched={touched.contractorId} name="contractorId" id="contractorId" />
                                            </div>

                                            <div className="linebylineInput valid-input fullInputWidth">
                                                <label className="control-label">{Resources['Notes'][currentLanguage]}</label>
                                                <div className={"inputDev ui input" + (errors.notes && touched.notes ? (" has-error") : !errors.notes && touched.notes ? (" has-success") : " ")} >
                                                    <div className="inputDev ui input">
                                                        <input autoComplete="off" className="form-control" value={values.notes} name="notes"
                                                            onBlur={(e) => {
                                                                handleBlur(e)
                                                                handleChange(e)
                                                            }} onChange={handleChange} placeholder={Resources['Notes'][currentLanguage]} />
                                                        {touched.notes ? (<em className="pError">{errors.notes}</em>) : null}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="doc-pre-cycle">
                                            <header>
                                                <h2 className="zero">{Resources['procurementContractorsItems'][currentLanguage]}</h2>
                                            </header>
                                            <ReactTable ref={(r) => { this.selectTable = r }} columns={columnsItem} defaultPageSize={10} minRows={2}
                                                data={this.state.SupplierData} noDataText={Resources['noData'][currentLanguage]} />
                                        </div>
                    ...
                    <div className="doc-container">
                        ...
                        <div className="step-content">
                            ...
                            <div className="step-content">
                                {this.state.FirstStep ?
                                    ...
                                    :
                                    <Fragment>{this.state.SecondStep ? StepTwo() : this.state.ThirdStep ? StepThree()
                                        : this.state.FourthStep ? StepFour() : StepFive()}
                                    </Fragment>
                                }
                            </div>
                        ...
                        <div className="docstepper-levels">
                            ...
                            <div className="step-content-foot">
                                <span onClick={this.PreviousStep} className={this.state.CurrStep !== 1 && this.state.docId !== 0 ? "step-content-btn-prev " :
                                ...
                                <div className="step-slider">
                                    <div onClick={this.StepOneLink} data-id="step1" className={'step-slider-item ' + (this.state.SecondStepComplate ? "active" : 'current__step')} >
                                        ...
                                        <div className="steps-info">
                                            <h6 onClick={e => this.setState({ CurrStep: 1 })}>{Resources['procurement'][currentLanguage]}</h6>
                                        </div>
                                    </div>
                                    ...
                                    <div onClick={this.StepTwoLink} data-id="step2 " className={'step-slider-item ' + (this.state.ThirdStepComplate ? 'active' : this.state.SecondStepComplate ? "current__step" : "")} >
                                        ...
                                        <div className="steps-info">
                                            <h6 >{Resources['procurementItems'][currentLanguage]}</h6>
                                        </div>
                                    </div>
                                    ...
                                    <div onClick={this.StepThreeLink} data-id="step3" className={'step-slider-item ' + (this.state.FourthStepComplate ? 'active' : this.state.ThirdStepComplate ? "current__step" : "")} >
                                        ...
                                        <div className="steps-info">
                                            <h6 >{Resources['procurementContractors'][currentLanguage]}</h6>
                                        </div>
                                    </div>
                                    ...
                                    <div onClick={this.StepFourLink} data-id="step4" className={'step-slider-item ' + (this.state.FivethStepComplate ? 'active' : this.state.FourthStepComplate ? "current__step" : "")} >
                                        ...
                                        <div className="steps-info">
                                            <h6 >{Resources['procurementContractorsItems'][currentLanguage]}</h6>
                                        </div>
                                    </div>
                                    ...
                                    <div onClick={this.StepFiveLink} data-id="step5" className={this.state.FivethStep ? "step-slider-item  current__step" : "step-slider-item"} >
                                        ...
                                        <div className="steps-info">
                                            <h6 >{Resources['ordering'][currentLanguage]}</h6>
                                        </div>
                                    </div>

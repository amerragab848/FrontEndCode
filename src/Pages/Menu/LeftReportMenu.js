    ...
    Router.map(route => {
      if (route.settings) {
        if (route.settings.OtherReports === true) {
          ...
          if (Config.IsAllow(route.settings.permission)) {
            ProjectReports.push({ label: Resources[route.title][currentLanguage], value: route.moduleId });
          }
        }
      }
    });
        ...
        <div className='settings-container reportsContainer'>
          ...
          <React.Fragment>
            <div className="settings-tabs-items">
              <h3 className="zero">{Resources.Reports[currentLanguage]}</h3>
              <ul className="zero" >
                <li data-tab="subiTab-1" className={(this.state[1] === true) ? "active" : " "} onClick={() => this.activeLi(1)} >
                  <span className="subUlTitle">{Resources.projectReports[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-2" className={(this.state[2] === true) ? "active" : " "} onClick={() => this.activeLi(2)}>
                  <span className="subUlTitle">{Resources.contractsPurchaseOrders[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-3" className={(this.state[3] === true) ? "active" : " "} onClick={() => this.activeLi(3)}>
                  <span className="subUlTitle">{Resources.tasksReports[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-4" className={(this.state[4] === true) ? "active" : " "} onClick={() => this.activeLi(4)}>
                  <span className="subUlTitle">{Resources.timeSheetReport[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-5" className={(this.state[5] === true) ? "active" : " "} onClick={() => this.activeLi(5)}>
                  <span className="subUlTitle">{Resources.otherReports[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-6" className={(this.state[6] === true) ? "active" : " "} onClick={() => this.activeLi(6)}>
                  <span className="subUlTitle">{Resources.inventoryRpt[currentLanguage]}</span>
                </li>
                <li data-tab="subiTab-7" className={(this.state[7] === true) ? "active" : " "} onClick={() => this.activeLi(7)}>
                  <span className="subUlTitle">{Resources.technicalOffice[currentLanguage]}</span>
                </li>
              </ul>
            </div>
            <div className="setting-tabs-contant">
              <div className="reports__menu">
                <label className="control-label">{this.state.moduleName}</label>
                <div className="reposrts__menu--dropdown" >
                  <Dropdown
                    data={this.state.subReports}
                    selectedValue={this.state.selectedReport}
                    handleChange={event => this.getReport(event)}
                    name="drop"
                    id="drop" />
                </div>
              </div>
